...
| `vendor` | `String` | Yes | `IBM` | Third-party software or hardware vendor associated with the CI. | Enables external SLA enforcement and vendor escalations for proprietary hardware/software. | **Categorical Feature** |
...
| `reassignment_count` | `Integer` | No | `1` | Number of times the ticket was reassigned across different assignment groups. | Measures routing friction and initial misassignment waste across support tiers. | **Operational Efficiency KPI / Post-Resolution Leakage** |
