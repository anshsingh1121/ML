...
graph TB
    subgraph Presentation["Presentation & Delivery Layer"]
        DASH["Streamlit Enterprise Dashboard (:8501)"]
    ...
    subgraph CoreML["Core Machine Learning & AI Layer"]
        ...
        EMB["Sentence Transformer (all-MiniLM-L6-v2)"]
...
| **Semantic Text Embeddings** | `SentenceTransformers` | Local execution of `all-MiniLM-L6-v2` (384-dim) providing fast (<20ms) and highly accurate semantic representation of technical incident text. |
...
- **Feature Registry (`src/data/feature_registry.py`)**: Stores 22 governance properties per column (`business_name`, `technical_name`, `target_leakage_classification`, `encoding_strategy`, `random_forest_usage`, `embedding_usage`, `faiss_metadata_usage`, `dashboard_usage`, `api_exposure`, `future_rag_usage`).
